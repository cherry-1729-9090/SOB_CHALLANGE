def little_endian(data):
  ...
  pairs.reverse()  
  return ''.join(pairs) 
# # formatted_version = "{:08x}".format(2)
# # print(formatted_version)
# # input_count = 2  
# # formatted_version = "{:02d}".format(input_count) 
# # print(formatted_version)

def conc_vin(transaction):
  concat_str = ""
  for vin in transaction['vin']:
    concat_str += vin['txid']+str(vin['vout'])+vin['scriptsig']+str(vin['sequence'])
  return concat_str
